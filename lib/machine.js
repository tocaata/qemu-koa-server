    ...
    this.eventHandlers = {
      SHUTDOWN: (data) => {
        ...
        if (guest) {
          ...
          broadcast('showMessage', { message: `Virtual machine ${this.name} is in shutdown.` });
        }
      },
      POWERDOWN: () => {
        const {guest} = data || {};
        ...
        if (guest) {
          ...
          console.log('powerdown is called');
          broadcast('showMessage', { message: `Virtual machine ${this.name} is in powerdown.` });
        }
      },
      STOP: () => {
      },
