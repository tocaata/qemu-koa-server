...
class Process {
  ...
  constructor(props, eventHandler) {
    this.proc = undefined;
  ...
  start(conf) {
    try {
      ...
      this.proc.on('error', (err) => {
        this.eventHandler['error'] && this.eventHandler['event']({ message: err });
      ...
      this.proc.on('exit', (code, signal) => {
        ...
        if (code === 0) {
          ...
          this.eventHandler['exit'] && this.eventHandler['exit']({ message: `Process has exited. ${ error }` });
          console.error(`QEMU-Process: exit with error: ${code}, signal: ${ signal }`);
        }
      });

      this.proc && this.proc.stderr.on('data', (data) => {
